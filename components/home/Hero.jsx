      ...
      <Container>
        ...
        <div className="bg-[#F8F6F2] rounded-2xl sm:rounded-3xl p-4 sm:p-8 lg:p-12 border border-light-beige shadow-subtle grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-12 items-center">
          ...
          <div className="lg:col-span-5 flex flex-col justify-center order-2 lg:order-1">
            <h1 className="hero-rise hero-d1 font-sans text-2xl sm:text-3xl lg:text-[40px] xl:text-[44px] font-extrabold text-charcoal tracking-tight leading-[1.2] mb-4 sm:mb-6">
              <span className="block mb-1 sm:mb-3">Jadikan Ruangmu</span>
              <span className="block">Terasa Seperti Rumah.</span>
            </h1>

            <p className="hero-rise hero-d2 text-sm text-warm-gray font-sans font-normal leading-relaxed max-w-lg mb-6 sm:mb-8">
              Modern living solutions home accessories and more exclusively at Edgar Space.
            </p>

            <div className="hero-rise hero-d3">
              <Button href="/koleksi" variant="terracotta" size="lg" className="btn-lift w-full sm:w-auto px-8 py-3.5 rounded-xl font-bold bg-terracotta hover:bg-terracotta-hover text-white transition-colors cursor-pointer shadow-xs">
                Jelajahi Koleksi
              </Button>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 order-1 lg:order-2">
            <div className="hero-clip relative aspect-[4/3] sm:aspect-[16/10] lg:aspect-[4/3] w-full rounded-xl sm:rounded-2xl overflow-hidden border border-light-beige shadow-subtle bg-soft-beige">
              <Image
        ...
        <div className="mt-4 sm:mt-6 grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          {features.map((feat, index) => {
            ...
            return (
              <div key={index} className="hero-rise hero-d4 bg-white p-4 sm:p-5 rounded-xl sm:rounded-2xl border border-light-beige shadow-xs flex items-center space-x-3 sm:space-x-4 transition-all duration-300 hover:shadow-subtle hover:border-terracotta/30" style={{ animationDelay: `${440 + index * 90}ms` }}>
